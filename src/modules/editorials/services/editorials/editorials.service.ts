...
export class EditorialsService {
  ...
  async getById(id: string): Promise<Editorial> {
    ...
    if (!items.length) {
      throw new HttpException(`Editorial with ID ${id} doesn't exist.`, HttpStatus.NOT_FOUND);
    }
    return items[0];
  }
  ...
  async create(name: string): Promise<Editorial> {
    const id = uuidv4();
    const result = await this._editorialsQB.create(id, name);
    if (!result) throw new HttpException(`There was an error creating the editorial`, HttpStatus.EXPECTATION_FAILED);
  ...
  async update(id: string, genre: Editorial): Promise<Editorial> {
    ...
    if (!result.affected) throw new HttpException(`There was an error updating the editorial`, HttpStatus.EXPECTATION_FAILED);
    return this.getById(id);
  }

  async delete(id: string): Promise<void> {
    const result = await this._editorialsQB.delete(id);
    if (!result.affected) throw new HttpException(`There was an error deleting the editorial`, HttpStatus.EXPECTATION_FAILED);
    return null;
  }
}
